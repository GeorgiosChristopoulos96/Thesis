...
def main_translation_process(data, model_name, language_pairs, sample_size_per_language, batch_size):
    for src, tgt in language_pairs:
        ...
        for original_index, translated_text in zip(subset.index, translated_texts):
            ...
            if tgt == 'ita_Latn':
                new_row['prefix'] = 'Italian'
            elif tgt == 'deu_Latn':
                new_row['prefix'] = 'German'
            elif tgt == 'fra_Latn':
                new_row['prefix'] = 'French'
            elif tgt == 'arb_Arab':
                new_row['prefix'] = 'Arabic'
            elif tgt == 'amh_Ethi':
                new_row['prefix'] = 'Amharic'
            elif tgt == 'glg_Latn':
                new_row['prefix'] = 'Galician'
            elif tgt == 'heb_Hebr':
                new_row['prefix'] = 'Hebrew'
            else:
                new_row['prefix'] = tgt
...
data_file_path = f"{PATH}/Datasets/WebNLG_MIXED(RU&EN)/combined_test.tsv"
output_file_path = f"{PATH}/Datasets/WebNLG_MIXED(RU&EN)/combined_test_DE_IT_FR.tsv"

data = load_data(data_file_path)
model_name = "facebook/nllb-200-distilled-600M"
language_pairs = [('en', 'deu_Ladata_augmentation_WebNLG.pytn'), ('en', 'ita_Latn'), ('en', 'fra_Latn'),('en','arb_Arab'), ('en','amh_Ethi'),('en','heb_Hebr'), ('en','glg_Latn')]
sample_size_per_language = 1102
